## Volve Field Production Analysis (Excel)

Production analysis of Equinor's Volve field (North Sea, 2008–2016), built in Excel from the daily/monthly production data for quick reconciliation with analysis with python. 
Oil was produced from the Middle Jurassic Hugin Formation under a water-injection waterflood.

![Dashboard Monthly -Excel.png](https://github.com/lindahafrifa-code/Volve-Field-Production-Analytics-Python-Jupyter/blob/main/excel/Dashboard%20Monthly%20-Excel.png)

## Key Findings
F-12 and F-14 produced ~85% of the field's ~10M Sm³ oil.
Calculated peak rates reconciled with NPD's reported field peak of ~9,000 Sm³/d.
Classic waterflood breakthrough as main producers reach ~97% water cut.
Late second-campaign wells watered up fast, consistent with drilling into a swept reservoir.
Rising field-wide water cut drove the 2016 shutdown, though pressure being maintained.


## Contents
Excel workbook (dashboard, KPIs, well-health summary)
Raw production dataset


## Data
Equinor Volve open dataset — CC BY-NC-SA 4.0. © Equinor, ExxonMobil, Bayerngas, NPD.
